...
class SdeNet(nn.Module):
    def __init__(self, sr, kernels, n_grus, features_set, att_conf):
        super(SdeNet, self).__init__()
        self.sr = sr
        self.n_fft = 1024 # 512
        self.hop_length = 512 # 256
        ...
        if self.features_set == "stft":
            self.data_in = [1, (10*sr+self.n_fft)//(self.n_fft - self.hop_length) - 1,  int(self.n_fft/2)]
        elif self.features_set == "sincos":
            self.data_in = [2, (10*sr+self.n_fft)//(self.n_fft - self.hop_length) - 1,  int(self.n_fft/2)]
        elif self.features_set == "all":
            self.data_in = [3, (10*sr+self.n_fft)//(self.n_fft - self.hop_length) - 1,  int(self.n_fft/2)]
        else:
            raise ValueError
...
class SdeTrainer(LightningModule):
    def __init__(self, lr, sr, kernels, n_grus, features_set, att_conf, **kwargs):
        super().__init__()        
        # Hyperparameters
        self.sr = sr
        self.kernels = kernels
        ...
        self.model = SdeNet(self.sr, self.kernels, self.n_grus, self.features_set, self.att_conf)
    ...
    def training_step(self, batch, batch_idx):
        audio, labels, _, _ = batch['audio'], batch['label'], batch['id'], batch['speech_id']
        ...
        self.log("train/loss_timewise", loss_timewise, prog_bar = True, on_epoch = True, on_step = False, sync_dist=True)
        self.log("train/mae", self.evaluate(distance_est, labels), on_epoch = True, prog_bar = True, on_step = False, sync_dist=True)
        return final_loss
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log("val/loss_timewise", loss_timewise, prog_bar = True, on_epoch = True, on_step = False, sync_dist=True)
        self.log("val/mae", self.evaluate(distance_est, labels), on_epoch = True, prog_bar = True, sync_dist=True)
        return final_loss
    
    def test_step(self, batch, batch_idx):
        audio, labels, ids, speech_ids = batch['audio'], batch['label'], batch['id'], batch['speech_id']
        distance_est, time_wise_distance, _ ,_ = self(audio)
        loss = self.loss(distance_est, labels)
        loss_timewise = self.loss(torch.mean(time_wise_distance, dim = -1), labels)
