...
def rename_files_in_dir(directory):
    for filename in os.listdir(directory):
        os.rename(os.path.join(directory, filename), os.path.join(directory, filename.replace("MeasurmentReceiver", "MeasurementReceiver")))
    ...
    for i in range(df.shape[0]):
        for j in range(df.shape[1]):
            ...
            if isinstance(df.iloc[i, j], str):
                df.iloc[i, j] = df.iloc[i, j].replace("SpatialReceiver", "MeasurementReceiver")
